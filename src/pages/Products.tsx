...
const Products = () => {
  ...
  const wooCommerceCategoryIds = useMemo(() => {
    ...
    if (loadingCategories) {
      console.log(`⏳ Categories still loading, will retry lookup for: "${effectiveCategoryParam}"`);
      ...
      if (knownCategoryIds[normalizedParam]) {
        console.log(`⚠️ Using fallback category ID ${knownCategoryIds[normalizedParam]} for "${effectiveCategoryParam}" (categories still loading)`);
    ...
    if (categories.length === 0 && !loadingCategories) {
      console.warn(`⚠️ Categories array is empty but not loading. Using fallback IDs for: "${effectiveCategoryParam}"`);
      ...
      const normalizedParam = effectiveCategoryParam.toLowerCase().trim();
      if (knownCategoryIds[normalizedParam]) {
        console.log(`✓ Using fallback category ID ${knownCategoryIds[normalizedParam]} for "${effectiveCategoryParam}"`);
        return [knownCategoryIds[normalizedParam]];
      }
    }
    ...
    if (categories.length > 0) {
      ...
      if (exactMatches.length > 0) {
        ...
        if (searchSlug === 'cooler') {
          ...
          if (laptopCooler) {
            ...
            console.log(`  ✓ Prioritized laptop accessories cooler (ID: 11790) over other cooler categories`);
        ...
        console.log(`✓ Found category by exact slug match: "${selectedMatch.name}" (ID: ${selectedMatch.id}, Slug: "${selectedMatch.slug}", Count: ${selectedMatch.count || 0})`);
      ...
      if (nameMatch) {
        ...
        console.log(`✓ Found category by name match: "${nameMatch.name}" (ID: ${nameMatch.id}, Slug: "${nameMatch.slug}")`);
        return categoryIds;
      }
    }
    ...
    if (categoryIds.length === 0 && categories.length > 0) {
      ...
      if (fuzzyMatch) {
        ...
        console.log(`✓ Found category by fuzzy match: "${fuzzyMatch.name}" (ID: ${fuzzyMatch.id}, Slug: "${fuzzyMatch.slug}")`);
      }
    }
    
    // Debug logging
    if (effectiveCategoryParam && categoryIds.length === 0) {
      console.error(`❌ Could not find WooCommerce category IDs for: "${effectiveCategoryParam}"`);
  ...
  //   console.log(`Category filter: "${categoryParam || 'none'}" → IDs: ${wooCommerceCategoryIds ? wooCommerceCategoryIds.join(', ') : 'NOT FOUND'}, Is Mapped: ${isMappedCategory}, Search: "${search || 'none'}", Search term: "${searchTerm || 'none'}"`);
  ...
  useEffect(() => {
    ...
    if (needsFallback) {
      ...
      if (!hasBagProducts) {
        console.log('🔧 Triggering bags fallback search...');
  ...
  const allFilteredProducts = useMemo(() => {
    ...
    if (isLaptopAccessoriesCooler) {
      ...
      console.log(`🔧 Laptop Cooler filter: ${beforeCount} → ${filtered.length} products (showing only laptop coolers)`);
    }
    ...
    if (isBagsCategory && !internalCategoryFilter && (filtered.length > 0 || shouldSearchAllForBags || bagsFallbackActive)) {
      ...
      if (bagProducts.length > 0) {
        ...
        console.log(`🔧 Bags filter: ${beforeCount} → ${filtered.length} products (found ${bagProducts.length} bag products)`);
        ...
        if (!shouldSearchAllForBags && !loading) {
          console.log(`🔧 Bags filter: ${beforeCount} → 0 products (all products were non-bag products). Will trigger fallback search...`);
          // Don't update state here - will be handled in useEffect
          ...
          filtered = [];
          console.log(`🔧 Bags filter: ${beforeCount} → 0 products (all products were non-bag products, fallback search also found no bags)`);
        }
      } else {
        ...
        console.log(`🔧 Bags filter: ${beforeCount} → ${filtered.length} products (removed ${nonBagProducts.length} non-bag products, kept ${filtered.length} ambiguous products)`);
        // Don't update state here - will be handled in useEffect
      }
      ...
      if (bagProducts.length > 0) {
        filtered = bagProducts;
        console.log(`🔧 Bags fallback search: ${beforeCount} → ${filtered.length} products (found ${bagProducts.length} bag products across all categories)`);
        // Don't update state here - will be handled in useEffect
      } else {
        // Remove non-bag products, keep the rest
        filtered = filtered.filter(product => !nonBagProducts.includes(product));
        console.log(`🔧 Bags fallback search: ${beforeCount} → ${filtered.length} products (removed ${nonBagProducts.length} non-bag products)`);
        // Don't update state here - will be handled in useEffect
      }
    }
